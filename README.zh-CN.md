...
<div align="center">
  <img src="docs/pipeline.svg" width="880" alt="FcMobile 管线：edl.json、clips/ 和公共 utils/ 引擎汇入 make_video.py，渲染出固定的「片头 → 可选数据卡 → 各段镜头 → Apple 风格片尾」结构（底下一条 BGM、原声静音），产出竖屏 9:16 与横屏 4K 两条成片。">
</div>

## 成片包含什么

每条成片 —— 竖屏和横屏 —— 都遵循上图这套固定结构：**片头（阵容全景 → 平滑缩放聚焦到本期球员卡）→ 可选的数据卡定格 → 各段游戏镜头（按 order 顺序，带嵌入式字幕）→ Apple 风格片尾。**
...
<div align="center">
  <img src="docs/layout-vertical.svg" width="700" alt="竖屏 9:16 版式解剖：16:9 游戏画面居中于上下纯黑边之间，字幕烧进金色圆角框。">
</div>
...
<div align="center">
  <img src="docs/cover-layout.svg" width="700" alt="封面版式：球员卡抠图在左、倾斜带金色外发光；真人照在右、左缘羽化融入暖色氛围 + 金色暗角背景。纯图无文字。">
</div>
...
> 游戏本体是 **[EA SPORTS FC™ Mobile](https://www.ea.com/games/ea-sports-fc/fc-mobile)**（免费，iOS / Android）。FcMobile 是非官方的粉丝自制剪辑管线，**与 EA 无隶属、赞助或背书关系**。本 README 里的所有图均为原创示意图，不含、也不转发任何 EA 游戏素材。

## 许可证

[MIT](LICENSE) © 2026 Siming Deng。许可证只覆盖源码和文档 —— 不覆盖你添加的任何素材。见 `LICENSE` 末尾的说明。

**致谢** —— README 中的示意图均为原创；图里的足球图标来自 [Twemoji](https://github.com/jdecked/twemoji)（© Twitter，采用 [CC-BY 4.0](https://creativecommons.org/licenses/by/4.0/) 许可）。
